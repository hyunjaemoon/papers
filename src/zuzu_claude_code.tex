...
\title{Sufficient Infrastructure for the One-Person, Two-Country Software Company: How ZUZU and Claude Code Let a Solo Founder Deliver Cloud Backend Systems Without an App Development Team}
...
Between 2021 and 2026 I ran an experiment, mostly involuntarily, on how much of that structure is actually load-bearing. Moonai (branded in Korean as \emph{Moon-AI}, with \emph{Moon} written in Hangul) is an AI delivery company targeting Korean small and medium-sized businesses, and it has exactly one person: me. Over those five years I attempted to build a portfolio of products --- an AI-powered industrial catalog (yushin-melamine.com), a social game-tracking platform (playshelfapp.com), an autonomous Python testing agent (pythontestingagent.com), consumer AI experiences, and the delivery business itself --- while holding a full-time engineering perspective shaped by prior industry work.

The honest result is asymmetric. I was \emph{not} able to provide app development at full product scale for the various Moonai products: the polish loop of consumer application development --- design iteration, platform-specific mobile work, the daily grind of UI refinement that separates a demo from a product people live in --- exceeds what one person can sustain across a portfolio. But I \emph{was} able to provide production-grade backend systems software on both GCP and AWS, deployed, monitored, multi-region, and serving real customers. The difference between those two outcomes was not talent or effort. It was that two specific pieces of infrastructure existed for one side of the work and not the other.
...
The first answer is Claude Code, the agentic coding tool I adopted as it matured over this period. The distinction between an AI code assistant and an agentic coding tool matters here. An assistant accelerates the person typing; an agent executes multi-step engineering work --- reading a codebase, writing code across many files, running commands, reading the errors, and iterating --- under human direction rather than human keystrokes.
...
\section{Lessons from Five Years}
